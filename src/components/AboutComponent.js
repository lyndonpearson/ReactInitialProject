...
function RenderPartner({sidekick}) {
    if (sidekick) {
        return (
            <React.Fragment>
                <Media object src={baseUrl + sidekick.image} alt={sidekick.name} width="150"/>
                <Media body className="ml-5 mb-4">
                    <Media heading>{sidekick.name}</Media>
...
function PartnerList(props) {
    const partnerJsx = props.partners.partners.map(aPartner => {
        return (
            <Fade in key={partnerJsx.id}>
                <Media tag="li" key={aPartner.id}><RenderPartner sidekick={aPartner} /></Media>
            </Fade>
        );
    });
